...
class ServerConnection(Connection):
    ...
    def connect(self, server, port=0, nickname="", password=None, username=None,
        ...
        threading.Thread(target=self._privmsg).start()
        return self
    ...
    def _handle_event(self, event):
        ...
        test = True
        try:
            authorhostmask = event.source().split('!')[1].split('@')[1]
            authornick = event.source().split('!')[0]
        ...
        if test:
            for i in [authorhostmask, authornick]:
    ...
    def _privmsg(self):
        while 1:
            if self.queue.qsize():
                ...
                if len(tosend[1]) > 442:
                    print ("yeah right")
                    self.queue.put([tosend[0], tosend[1][441:]])
                    tosend[1] = tosend[1][:441]
                    
                self.send_raw("PRIVMSG %s :%s" % (tosend[0], tosend[1]))
                time.sleep(2)
    ...
    def send_raw(self, string):
        ...
        try:
            ...
            if DEBUG and not "PING" in string:
