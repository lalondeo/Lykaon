...
class OneUseCommandPlayer(Villager):
    ...
    def runcommand(self, target):
        if target == self.name:
            ...
            raise Game.WerewolfException(Game.ALREADYUSEDCMD.format(self.OPERATIONNAME))
...
class Seer(OneUseCommandPlayer):
    ...
    def _cmd(self, target):
        ...
        print Game.MSG_SEER.format(target, kind)
...
class Detective(OneUseCommandPlayer):
    ...
    def _cmd(self, target):
        return Game.MSG_DET.format(target, self.game.PlayerList[target].name_singular)
